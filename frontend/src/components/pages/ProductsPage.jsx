      ...
      <div className="min-h-screen bg-gray-100 py-6 px-4 pb-32">
        <div className="max-w-6xl mx-auto flex gap-4">
          ...
          <div className="flex-1">
            ...
            {loading && <div className="text-center py-20 text-gray-400 font-medium animate-pulse">Fetching fresh items...</div>}
            {error && <div className="text-red-600 p-4 bg-red-50 rounded-xl border border-red-100">{error}</div>}
            ...
            {!loading && !error && (
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {products.map(p => {
                  ...
                  return (
                    <div key={p._id} className="bg-white rounded-2xl border border-gray-100 p-3 flex flex-col hover:shadow-lg transition-all group">
                      <div className="relative overflow-hidden rounded-xl mb-3">
                         <img src={p.imageUrl || "https://via.placeholder.com/150"} alt={p.name} className="w-full h-36 object-contain group-hover:scale-105 transition-transform" />
                      </div>
                      
                      <div className="flex-1">
                        <div className="font-bold text-sm text-gray-800 line-clamp-2 leading-tight h-10">{p.name}</div>
                        <div className="text-[10px] text-gray-400 font-bold uppercase mt-1 tracking-widest">{p.category}</div>
                      </div>

                      <div className="flex items-center justify-between mt-4">
                        <div className="font-black text-gray-900 text-lg">₹{p.price}</div>
                        
                        {qty > 0 ? (
                          <div className="flex items-center bg-green-700 text-white rounded-lg shadow-inner overflow-hidden">
                            <button onClick={() => updateQuantity(p, -1)} className="px-3 py-1.5 hover:bg-green-800 font-bold">−</button>
                            <span className="px-1 text-xs font-bold w-6 text-center">{qty}</span>
                            <button onClick={() => updateQuantity(p, 1)} className="px-3 py-1.5 hover:bg-green-800 font-bold">+</button>
                          </div>
                        ) : (
                          <button 
            ...
            {!loading && products.length > 0 && (
              <div className="mt-10 flex items-center justify-center gap-3">
                <button disabled={page <= 1} onClick={() => setPage(p => Math.max(1, p - 1))} className="px-4 py-2 bg-white border rounded-xl disabled:opacity-30 shadow-sm font-medium">Prev</button>
                <div className="text-sm font-bold text-gray-500 bg-white px-4 py-2 rounded-xl border">Page {page} / {pages}</div>
                <button disabled={page >= pages} onClick={() => setPage(p => Math.min(pages, p + 1))} className="px-4 py-2 bg-white border rounded-xl disabled:opacity-30 shadow-sm font-medium">Next</button>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="md:hidden mt-8 overflow-x-auto no-scrollbar flex gap-2 pb-4">
           <button onClick={()=>{setSelectedCategory(""); setPage(1);}} className={`px-4 py-2 rounded-full whitespace-nowrap text-sm font-bold shadow-sm ${selectedCategory==="" ? "bg-green-700 text-white" : "bg-white text-gray-600"}`}>All</button>
           <MobileCategories setSelectedCategory={setSelectedCategory} setPage={setPage} current={selectedCategory} />
        </div>
      </div>
          ...
          >
            <div className="flex items-center gap-4">
               <div className="flex flex-col">
                  <span className="text-[10px] font-black opacity-70 uppercase tracking-tighter">{cartCount} {cartCount === 1 ? 'Item' : 'Items'}</span>
                  <span className="text-lg font-black leading-none">₹{cartTotal}</span>
               </div>
            </div>
...
function MobileCategories({ setSelectedCategory, setPage, current }) {
  ...
  return (
    <>
      {cats.map(c => (
        <button key={c.category} onClick={()=>{ setSelectedCategory(c.category); setPage(1); }} className={`px-4 py-2 rounded-full whitespace-nowrap text-sm font-bold shadow-sm transition-all ${current === c.category ? "bg-green-700 text-white" : "bg-white text-gray-600"}`}>
          {c.category}
        </button>
      ))}
    </>
