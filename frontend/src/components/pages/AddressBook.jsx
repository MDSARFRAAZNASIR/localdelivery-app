    ...
    <>
      ...
      <div className="max-w-3xl mx-auto p-4 sm:p-8 bg-white min-h-screen">
        <header className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-black text-gray-900 tracking-tight">{t("Address Book")}</h1>
            <p className="text-gray-400 font-bold text-xs uppercase tracking-widest mt-1">{t("Manage delivery locations")}</p>
          </div>
          {mode === "manage" && (
            <button
        ...
        {loading && <div className="text-center py-10 font-black text-gray-200 animate-pulse">{t("Loading Addresses")}...</div>}
              ...
              <div className="flex items-start justify-between">
                ...
                {mode === "manage" && (
                  <div className="flex flex-col gap-2">
                    ...
                    }} className="text-[10px] font-black text-orange-600 hover:underline">{t("EDIT")}</button>
                    {!addr.isDefault && (
                      <button onClick={(e) => { e.stopPropagation(); setDefault(addr._id); }} className="text-[10px] font-black text-blue-600 hover:underline">{t("DEFAULT")}</button>
                    )}
                    <button onClick={(e) => { e.stopPropagation(); deleteAddress(addr._id); }} className="text-[10px] font-black text-red-600 hover:underline">{t("DELETE")}</button>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-[32px] p-6 sm:p-8 border border-gray-100 shadow-inner space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                   <label className="text-[10px] font-black text-gray-400 uppercase mb-1 ml-1 block">{t("Address Label")}</label>
                ...
                <input placeholder={t("Receiver's Name")} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="col-span-2 sm:col-span-1 bg-white border-none rounded-2xl p-4 text-sm font-bold shadow-sm focus:ring-2 focus:ring-orange-500/20" />
              ...
              <div className="relative">
                ...
                {isVerified && <span className="absolute right-4 top-4 text-green-600 text-xs font-bold">✔ {t("Verified")}</span>}
              </div>
        ...
        <span className="text-[10px] font-black text-gray-300">
          {t("RESEND IN")} {timer}s
            ...
            <button onClick={() => setShowMap(true)} className="w-full py-6 border-2 border-dashed border-gray-100 rounded-[32px] text-gray-400 font-black text-xs uppercase tracking-widest hover:border-blue-200 hover:text-blue-500 transition-all">
              📍 {t("Use Interactive Map to Pin Location")}
            </button>
          )}
        </div>
          ...
          <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
             <div className="bg-white w-full max-w-2xl rounded-[32px] overflow-hidden shadow-2xl">
               <div className="p-4 border-b flex justify-between items-center bg-gray-50">
                  <span className="font-black text-xs uppercase">{t("Move pin to your doorstep")}</span>
